...
free_answer_questions = [
    ...
    utils.Question(
        body="Какой модный дом был основан во второй половине двадцатого века женщиной, которая стремилась привнести в женский гардероб различные элементы мужского? Подсказка: духи, производимые её компанией, были бестселлером прошлого века. ",
        answers=[utils.AnswerOption(number=1, text="Шанель", is_correct=True)],
        right_text="Коко Шанель действительно внесла огромный вклад в мировую моду!",
        wrong_text="Нет, это Коко Шанель! Она популяризировала приталенные жакеты, сумки на цепочках и сделала популярнейшую линейку духов «шанель номер пять»."
    ),
    utils.Question(
        body="В 1969 году в Париже был открыт самый большой магазин макияжа и красоты. Как назывался этот магазин?",
        answers=[utils.AnswerOption(number=1, text="Сефора", is_correct=True)],
        right_text="Именно!",
        wrong_text="А вот и нет! Это был магазин Сефора!"
    )
]
...
multiple_choice_questions = [
    ...
    utils.Question(
        body="Как называется явление, при котором в кризисные времена люди имеют тенденцию баловать себя премиальными покупками?",
        answers=[utils.AnswerOption(number=1, text="эффект помады", is_correct=True),
                 utils.AnswerOption(number=2, text="закон чёрной икры", is_correct=False),
                 utils.AnswerOption(number=3, text="правило золотого зуба", is_correct=False),
                 utils.AnswerOption(number=4, text="закон шёлкового платья", is_correct=False)],
        right_text="Верно! Если не знать об этом, то можно здорово прогадать на котировках акций! Как считают исследователи из Американской психологической ассоциации, причина кроется в женской психологии, а именно в стремлении повысить свою привлекательность в условиях обострения конкуренции за обеспеченных представителей противоположного пола, число которых снижается во время рецессий.",
        wrong_text="Мимо! Это явление называется эффект помады. Если не знать об этом, то можно здорово прогадать на котировках акций! Как считают исследователи из Американской психологической ассоциации, причина кроется в женской психологии, а именно в стремлении повысить свою привлекательность в условиях обострения конкуренции за обеспеченных представителей противоположного пола, число которых снижается во время рецессий."
    ),
]
